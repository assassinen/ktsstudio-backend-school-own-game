...
class GameManager:
    def __init__(self, app: "Application"):
        ...
        self.time_handlers = {
            ...
            # GameStatus.select_theme: self._time_mock,
            GameStatus.ask_question: self._time_mock,
        }
    ...
    async def check_select_theme(self, game):
        if datetime.now() - game.update_date > timedelta(seconds=self.app.config.game_settings.time_select_theme):
            ...
            # el
            if iteration >= self.app.config.game_settings.number_questions:
    ...
    async def game_handler_select_theme(self, update):
        ...
        if update.object.text.endswith('необходимо выбрать тему:') and current_user.uuid == last_game.active_user:
            await self.app.store.game_accessor.update_game(last_game.uuid,
                                                           {'status': GameStatus.ask_question,
                                                            'round': last_game.round+1})
            ...
            question = (
                random.choice([(theme.title, theme.uuid) for theme
                               in await self.app.store.game_accessor.get_questions_by_theme_uuid(update.object.data)]))

            await self.app.store.game_accessor.insert_question_to_game(game_uuid=last_game.uuid,
            ...
            message = Message(chat_id=last_game.chat_id,
                              text=question[0],
                              inline_data=[inline_data])
        else:
            themes = {theme.uuid: theme.title for theme in await self.app.store.game_accessor.get_themes()}
    ...
    async def game_handler_ask_question(self, update):
        ...
        question_by_game = await self.app.store.game_accessor.get_question_by_game_uuid_and_round(last_game.uuid,
                                                                                                  last_game.round)
        ...
        if (update.type == 'callback_query'
            ...
            answer_to_game = await self.app.store.game_accessor.get_answer_to_game(last_game.uuid,
                                                                                   update.object.data,
                                                                                   user.uuid,
                                                                                   last_game.round)
            if answer_to_game is None:
                await self.app.store.game_accessor.insert_answer_to_game(last_game.uuid,
                                                                         update.object.data,
                                                                         user.uuid,
                                                                         last_game.round)
